...
**Base URL**: `https://your-domain.com` (Production) / `http://192.168.0.182:8008` (Development)
...
class RushHourAPI {
  ...
  constructor(baseURL: string = 'http://192.168.0.182:8008') {
...
#### Health Check
```bash
curl -X GET http://192.168.0.182:8008/api/v1/health
```

#### Google Login
```bash
curl -X POST http://192.168.0.182:8008/api/v1/auth/google \
...
#### Get User's Cars
```bash
curl -X GET "http://192.168.0.182:8008/api/v1/user-car/by-user-id?userId=123" \
  -H "Authorization: Bearer your_jwt_token_here"
```

#### Create Car Blocking
```bash
curl -X POST http://192.168.0.182:8008/api/v1/car-relations \
  -H "Authorization: Bearer your_jwt_token_here" \
  -H "Content-Type: application/json" \
  -d '{
    "blockingCarId": 456,
...
const API_BASE_URL = 'http://192.168.0.182:8008';
```
