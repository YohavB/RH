...
export const googleLogin = async (
  ...
  let existingUser = mockUsers.find(
    (user) => user.email === mockUserInfo.email
  );
...
export const findOrCreateCar = async (
  plateNumber: string,
  country: Countries,
  userId?: number
): Promise<CarDTO> => {
  ...
  let car = mockCars.find(
    (c) => c.plateNumber === plateNumber && c.country === country
  );
...
export const getUserCars = async (userId: number): Promise<UserCarsDTO> => {
  ...
  const userCars = mockCars.filter((car) =>
    mockUsersCars.some(
      (uc) => uc.userId === userId && uc.userCar === car.plateNumber
    )
  );
...
export const removeCarFromUser = async (
  userId: number,
  carId: number
): Promise<UserCarsDTO> => {
  ...
  const remainingCars = mockCars.filter(
    (car) =>
      car.id !== carId &&
      mockUsersCars.some(
        (uc) => uc.userId === userId && uc.userCar === car.plateNumber
...
export const getCarRelations = async (
  carId: number
): Promise<CarRelationsDTO> => {
...
export const sendNeedToGoNotification = async (
  blockedCarId: number
): Promise<NotificationResponse> => {
  logApiCall(
    "POST",
    `/api/v1/notification/send-need-to-go?blockedCarId=${blockedCarId}`
  );
  await simulateDelay();
...
export const findCarByPlateNumber = async (
  plateNumber: string,
  country: Countries
): Promise<any> => {
  console.warn(
    "findCarByPlateNumber is deprecated. Use findOrCreateCar instead."
  );
...
export const saveCar = async (
  ...
  console.warn(
    "saveCar is deprecated. Use findOrCreateCar and assignCarToUser instead."
  );
  ...
  const existingCarIndex = mockCars.findIndex(
    (c) => c.plateNumber === carInfo.plateNumber
  );
...
export const getAllUsersCars = async (): Promise<any> => {
  console.warn(
    "getAllUsersCars is deprecated. Use getUserCars with specific userId instead."
  );
...
export const getUsersCarsByPlateNumber = async (
  plateNumber: string
): Promise<any[]> => {
  console.warn(
    "getUsersCarsByPlateNumber is deprecated. Use findOrCreateCar and getCarRelations instead."
  );
  logApiCall("POST", "/api/users-cars/car", { params: { plateNumber } });
  await simulateDelay();

  const filteredUsersCars = mockUsersCars.filter(
    (uc) => uc.userCar === plateNumber
  );
...
export const getUsersCarsByUserAndPlate = async (
  userId: number,
  plateNumber: string
): Promise<any> => {
  console.warn(
    "getUsersCarsByUserAndPlate is deprecated. Use findOrCreateCar and getUserCars instead."
  );
  logApiCall("POST", "/api/users-cars/by-user-and-plate", {
    params: { userId, plateNumber },
  });
  await simulateDelay();

  const userCar = mockUsersCars.find(
    (uc) => uc.userId === userId && uc.userCar === plateNumber
  );

  if (!userCar) {
    throw new Error(
      `User car not found for user ${userId} and plate ${plateNumber}`
    );
  }
...
export const getUsersCarsByBlockedPlateNumber = async (
  blockedPlateNumber: string
): Promise<any> => {
  console.warn(
    "getUsersCarsByBlockedPlateNumber is deprecated. Use getCarRelations instead."
  );
  logApiCall("POST", "/api/users-cars/blocked", {
    params: { blockedPlateNumber },
  });
  await simulateDelay();

  const filteredUsersCars = mockUsersCars.filter(
    (uc) => uc.blockedCar === blockedPlateNumber
  );
...
export const getUsersCarsByBlockingPlateNumber = async (
  blockingPlateNumber: string
): Promise<any> => {
  console.warn(
    "getUsersCarsByBlockingPlateNumber is deprecated. Use getCarRelations instead."
  );
  logApiCall("POST", "/api/users-cars/blocking", {
    params: { blockingPlateNumber },
  });
  await simulateDelay();

  const filteredUsersCars = mockUsersCars.filter(
    (uc) => uc.blockingCar === blockingPlateNumber
  );
...
export const updateBlockedCarByPlateNumber = async (
  ...
  console.warn(
    "updateBlockedCarByPlateNumber is deprecated. Use createCarBlockingRelationship instead."
  );
  ...
  const userCarIndex = mockUsersCars.findIndex(
    (uc) => uc.userId === userId && uc.userCar === blockingCarPlate
  );
  if (userCarIndex >= 0) {
...
export const releaseBlockedCarByPlateNumber = async (
  ...
  console.warn(
    "releaseBlockedCarByPlateNumber is deprecated. Use removeCarBlockingRelationship instead."
  );
...
export const findAllUsers = async (): Promise<any> => {
  console.warn(
    "findAllUsers is deprecated. Use getUserById or getUserByEmail instead."
  );
...
export const findUserByExternalId = async (
  ...
  console.warn(
    "findUserByExternalId is deprecated. This endpoint is not available in the new API."
  );
...
export const createOrUpdateUser = async (userDTO: any): Promise<any> => {
  console.warn(
    "createOrUpdateUser is deprecated. Use createUser or updateUser instead."
  );
  if (userDTO.id) {
